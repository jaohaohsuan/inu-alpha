...
case class JStoredQuery(title: String, tags: Option[String])
...
class StoredQueryAggregateRootView extends MaterializeView {
  ...
  def convertToReadSideType(value: StoredQuery) = {
    ...
    val data = JField("data", JStoredQuery(title, tags: Option[String]))
    ...
    storedQueryId -> compact(render(JObject(fields:_*)))
  }
  ...
  def receive: Receive = {
    case "GO" =>
      ...
      source.mapAsync(1)(saveOrUpdate).runForeach(println)
      //.runWith(Sink.ignore)
  }
}
